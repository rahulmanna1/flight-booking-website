...
import { Clock, Plane, MapPin, Calendar, Users, Info, Wifi, Utensils, Monitor, Zap, Eye, Loader2 } from 'lucide-react';
...
export default function FlightCard({ flight, onSelect, searchData }: FlightCardProps) {
  ...
  // Handle flight selection with loading state
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (isSelectingFlight) return; // Prevent actions during loading
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg ${airlineInfo.color}`} aria-hidden="true">
            {airlineInfo.icon}
          </div>
          <div>
            <h3 className="font-bold text-lg text-gray-900" id={`flight-details-${flight.id}`}>{flight.airline}</h3>
            <p className="text-sm text-gray-500" aria-label={`Flight number ${flight.flightNumber}, aircraft ${flight.aircraft}`}>{flight.flightNumber} • {flight.aircraft}</p>
            <div className="flex items-center space-x-2 mt-1">
              <p className="text-xs text-gray-500">{aircraftInfo.category} Aircraft</p>
              <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${travelClassInfo.color}`} aria-label={`Travel class: ${travelClassInfo.label}`}>
                <span aria-hidden="true">{travelClassInfo.icon}</span>
                <span className="ml-1">{travelClassInfo.label}</span>
              </span>
            </div>
          </div>
        </div>
        <div className="text-right">
          ...
          <div className="flex items-center justify-end mt-1">
            ...
            <span className="text-xs text-gray-500">Adult fare</span>
          </div>
        </div>
      </div>

      {/* Flight Route */}
      <div className="grid grid-cols-3 gap-6 mb-6" role="group" aria-label="Flight route and timing information">
        {/* Departure */}
        <div className="text-left">
          ...
          <p className="text-2xl font-bold text-gray-900 mb-1" aria-label={`Departure time ${flight.departTime}`}>{flight.departTime}</p>
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-bold text-gray-900 mb-1" aria-label={`Arrival time ${flight.arriveTime}`}>{flight.arriveTime}</p>
